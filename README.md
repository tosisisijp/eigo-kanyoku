...
#### マスターファイル参照型システムへの移行と`back`関連ファイルの全面改訂

**背景**: これまでのルールや個別ファイルがそれぞれ情報を持ち、メンテナンスコストの増大と不整合のリスクを抱えていました。特に`back`関連の表現は数が多く、体系的な管理が急務でした。

**実装成果**:
1.  **マスターファイルの確立**: `tango/back.md` を唯一の情報源（Single Source of Truth）として定義。ここにTOEIC優先度、Ankiラベル、機能別分類、標準文例を集約しました。
2.  **機能別分類の導入**: 学習効果を最大化するため、従来の文法分類だけでなく「コミュニケーション系」「行動操作系」といった実践的な機能別分類を導入しました。
3.  **参照型ルールへの移行**: `204`番、`205`番などのルールから具体的な表現リストを削除し、`tango/back.md` を参照する形にシステムを再設計。これにより、マスターファイルを更新するだけで関連ルールが自動的に追従するようになりました。
4.  **`back`関連ファイルの全面改訂**: 上記の新しいマスターファイルとルールに基づき、`back-*.md` の個別ファイルを全面改訂。Anki IDを保護しながらファイル名をリネームし、内容をマスターファイルと完全に同期させました。
5.  **命名規則とID保護のルール化**: ファイルリネーム時の`Move-Item`コマンド使用の徹底など、Anki ID保護の重要性を`201-Ankiデッキ構造管理.mdc`と`202-Obsidian_to_Anki独自形式管理.mdc`に明記しました。

**プロジェクトへの価値**:
-   **メンテナンス性の劇的な向上**: `tango/back.md` を更新するだけで、関連ファイルやルールの整合性が保たれるようになりました。
-   **学習効率の最適化**: 機能別分類とTOEIC優先度により、より戦略的な学習が可能になりました。
-   **システムの堅牢性強化**: 単一情報源の原則を導入したことで、情報の不整合リスクが大幅に低減しました。
...
#### 202番ルール絶対的準拠による句動詞分類修正（2025年06月28日最新）
**背景**: 先の句動詞分類体系修正で使用した「不要補語副詞句動詞」というAnkiラベルが、hinshi/句動詞.mdの末端品詞比較表に存在しない独自ラベルであることが判明

**問題の発覚**:
- **不適切ラベル使用**: 「不要補語副詞句動詞」はhinshi/句動詞.mdの分類体系に存在しない
- **202番ルール違反**: 品詞分析・Ankiラベル管理の絶対的準拠原則への抵触
- **システム不整合**: 末端品詞比較表と実装の乖離による学習効率低下

**修正実装成果**:
- **正しい分類判定**: 「go back」「get back」は分離不可能 → 不要補語固定句動詞に修正
- **完全整合性確保**: hinshi/句動詞.mdの末端品詞比較表との100%一致達成
- **3ファイル修正完了**:
  - go-back-戻る.md: 全セクションでの「不要補語固定句動詞」統一
  - get-back-連絡する.md: 全セクションでの「不要補語固定句動詞」統一  
  - tango/back.md: Ankiラベル分類一覧表・実用文例・慣用表現リストの統一
- **分離可能性テスト完了**: "go it back" / "get it back" → 分離不可の確認済み

**202番ルール絶対的準拠原則の確立**:
- **末端品詞比較表準拠**: hinshi/フォルダの比較表にあるAnkiラベルのみ使用可能
- **独自作成絶対禁止**: 末端品詞比較表にないラベルの作成・使用を完全禁止
- **完全一致原則**: 表記・大文字小文字・記号まで末端品詞比較表と完全一致必須
- **品質保証体制**: 今後の全Ankiラベル設定での厳格なチェック体制確立

**学習システム価値向上**:
- **正確性向上**: 句動詞分類の文法的正確性による学習効果最大化
- **一貫性確保**: 全プロジェクトでの統一された分類体系による混乱防止
- **信頼性確立**: hinshi/システムとの完全整合による教材としての信頼性向上

#### 202番ルール形式統一とObsidian_to_Anki品質強化
**背景**: Obsidian_to_Ankiセクションのフォーマット不統一問題の発見と解決

**実装成果**:
- **フォーマット統一**: 11個のback関連ファイルで202番ルール準拠に修正
  - 修正対象: back-away, back-off, back-in-the-day, turn-ones-back-on, set-back, scale-back, pull-back, get-someone-back, date-back-to, behind-someones-back, back-背中
  - 統一形式: `eigo-kanyoku` → `[表現]` → 空行 → `[英語例文]` → `Back:` の正しい構造に修正
  - 問題形式: `eigo-kanyoku` → `Yaku:` → `Hyogen:` の旧形式から脱却
- **Anki連携最適化**: 正しいフォーマットによるObsidian_to_Ankiプラグインとの互換性向上
- **学習効率改善**: 統一されたカード作成形式による学習体験の向上

#### デバッグ機能強化とプラグイン最適化
**背景**: Obsidian_to_Ankiプラグインの動作解析とトラブルシューティング機能強化

**実装成果**:
- **詳細デバッグ機能**: `.obsidian/plugins/obsidian-to-anki-plugin/main.js`のscanNotesメソッドにID付きデバッグメッセージ追加
  - [DEBUG-SN001]～[DEBUG-SN014]: 14個のデバッグポイントによる詳細な実行フロー追跡
  - for文・if文の開始前、内部、終了直後の完全な状態監視
  - 各処理段階での変数値・配列カウント・識別子状態の記録
- **ログ出力最適化**: console.debugからconsole.logに変更による視認性向上
- **トラブルシューティング**: プラグイン動作の詳細分析が可能になり、問題の早期発見・解決を実現

**効果・価値**:
- **品質向上**: 11ファイルの完全な形式統一によるAnkiカード作成品質の向上
- **学習効率**: 統一されたフォーマットによる学習体験の改善
- **保守性向上**: デバッグ機能によるプラグイン問題の迅速な特定・解決
- **開発効率**: 詳細なログによる機能改善・最適化の基盤構築

### 最新の主要改善（2025年07月03日）

#### PowerShell操作ガイドの根本的改善とページャー問題解決
**背景**: PowerShellでのGitコマンド実行時に発生するページャーフリーズ問題の根本的解決が必要

**実装成果**:
- **004-PowerShell操作ガイド.mdc**: ページャー回避設定の根本的改善
  - 従来: `| Write-Output` や `| cat` による回避策
  - 新方式: `git config --global core.pager cat` による根本解決
  - 設定後は通常のGitコマンドがそのまま使用可能
- **文字エンコーディング問題の解決**: `git cat-file blob` コマンドによる日本語ファイル表示の最適化
  - `git show` での文字化け問題を完全解決
  - 日本語ファイルのコミット版確認が正常に動作
- **ルール体系の簡素化**: 複雑な回避策からシンプルな設定による解決へ移行
  - ページャー回避コマンドセクションの削除
  - グローバル設定による根本解決方法の推奨

**技術的価値**:
- **操作性向上**: 通常のGitコマンドがそのまま使用可能
- **安定性向上**: ページャーフリーズ問題の完全解決
- **保守性向上**: シンプルな設定による運用負荷軽減
- **学習効率**: 日本語ファイルの履歴確認が正常に動作

#### AnkiカードID管理の最適化
**背景**: Obsidian to Ankiプラグインが自動生成するIDコメントの適切な管理

**実装成果**:
- **IDコメントの整理**: 不要なIDコメントの削除によるファイル簡素化
  - `scale-back-縮小する.md` から `<!--ID: 1751241922002-->` を削除
  - プラグインが自動生成するIDの適切な管理
- **ファイル構造の最適化**: 必要最小限の情報のみ保持
  - Obsidian_to_Ankiセクションの構造維持
  - 関連表現リンクの保持
  - 学習効果に影響しない範囲での整理

**学習システム価値**:
- **ファイル簡素化**: 不要な情報の削除による可読性向上
- **管理効率**: IDコメントの適切な管理による保守性向上
- **品質維持**: 学習効果に影響しない範囲での最適化
...
#### 202番ルール形式統一とObsidian_to_Anki品質強化（2025年01月28日最新）
- **フォーマット統一**: 11個のback関連ファイルで202番ルール準拠に修正
  - 統一形式: `eigo-kanyoku` → `[表現]` → 空行 → `[英語例文]` → `Back:` の正しい構造
  - 修正対象: back-away, back-off, back-in-the-day, turn-ones-back-on, set-back, scale-back, pull-back, get-someone-back, date-back-to, behind-someones-back, back-背中
  - 旧形式排除: `eigo-kanyoku` → `Yaku:` → `Hyogen:` から脱却
- **Anki連携最適化**: 正しいフォーマットによるObsidian_to_Ankiプラグインとの互換性向上
- **学習効率改善**: 統一されたカード作成形式による学習体験の向上
- **品質保証**: 全ファイルの202番ルール準拠確認完了

#### デバッグ機能強化とプラグイン最適化（2025年06月28日最新）
- **詳細デバッグ機能**: `.obsidian/plugins/obsidian-to-anki-plugin/main.js`のscanNotesメソッドにID付きデバッグメッセージ追加
  - [DEBUG-SN001]～[DEBUG-SN014]: 14個のデバッグポイントによる詳細な実行フロー追跡
  - for文・if文の開始前、内部、終了直後の完全な状態監視
  - 各処理段階での変数値・配列カウント・識別子状態の記録
- **ログ出力最適化**: console.debugからconsole.logに変更による視認性向上
- **トラブルシューティング**: プラグイン動作の詳細分析が可能になり、問題の早期発見・解決を実現
- **保守性向上**: デバッグ機能によるプラグイン問題の迅速な特定・解決基盤を構築

#### 202番ルール形式標準化とObsidian_to_Anki最適化（2025年06月）
...
**最終更新**: 2025年07月10日
**プロジェクト状況**: アクティブ開発中
**主要成果**: 句動詞・慣用句リスト拡張（No.11-26追加）・個別Ankiカードファイル作成（16件）・改行コードLF統一・202番ルール形式統一・Obsidian_to_Anki品質強化・デバッグ機能強化・プラグイン最適化・Git操作制御システム導入・4パターンフロー制御確立・コミット前README確認システム確立・Git差分統合システム確立・ルール体系3層構造化・日付管理標準化・README更新プロセス自動化
**最新の技術的突破**: 句動詞・慣用句No.11-26の追加と16個の個別Ankiカードファイル作成による学習コンテンツ大幅拡張

## 更新履歴（内部管理用）
- 2025-07-01: Ankiカードでの読み上げ互換性向上のため、Ankiラベルからアイコンを削除。対象はルート直下の表現ファイル、tango/back.md, tango/hinshi/句動詞.md。
...
#### Ankiカード検証・修正スクリプトの追加とback関連表現の形式統一強化

**実装成果**:
1. **自動検証スクリプト群の追加**: `script/AnkiCardValidator.ps1`, `script/AnkiCardValidatorV2.ps1`, `script/AnkiCardCorrector.ps1` を新規作成。Obsidian_to_Anki形式の自動検証・修正を実現し、手動確認負荷を大幅軽減。
2. **検証結果ログ**: `script/result.txt`, `script/resultV2.txt` を生成し、実行結果を履歴として保存。
3. **back関連表現ファイルのマイナー修正**: 25ファイルで日本語訳の微調整とタグ位置を統一。
4. **デバッグログ改善**: `.obsidian/plugins/obsidian-to-anki-plugin/main.js` に追加したデバッグポイントを最適化し、実行パフォーマンスを維持しつつ詳細分析を可能に。

**プロジェクトへの価値**:
- **品質保証自動化**: クリック一つで全カードの形式不備を検出・修正可能に。
- **学習効率向上**: 表現ファイルの統一フォーマットによりAnkiカード生成エラーを完全排除。
- **保守性強化**: スクリプトとログで変更履歴を可視化し、将来の拡張に備えた基盤を整備。
