...
#### 句動詞分類のさらなる整合性向上と新ラベル導入

**背景**: 「get back to」の分類が `必須補語自的固定句動詞` として扱われていましたが、実際には *to* 句を伴うコミュニケーション行為を表すことから、`コミュニケーション系前置詞句動詞` として統一する必要がありました。

**実装成果**:
1. **新ラベル導入**: 異なるコミュニケーション行為を包括するラベル「コミュニケーション系前置詞句動詞」を末端品詞比較表および Anki ラベル一覧に正式追加。
2. **マスターファイル修正**: `tango/back.md` の Anki ラベル分類一覧表に新ラベル行を追記し、No.3 **get back to** のラベルを更新。
3. **個別ファイル統一**: `get-back-to-連絡する.md` の frontmatter、Obsidian_to_Anki セクション、Tags を新ラベルに置き換え。
4. **整合性検証**: `hinshi/句動詞.md` のツリー分類とラベル表記の一貫性を再確認し、完全一致を確認。

**プロジェクトへの価値**:
- **一貫性向上**: コミュニケーション行為に関連する句動詞を明確にカテゴライズし、学習者および AI の検索性を強化。
- **学習効率改善**: TOEIC ビジネス連絡シーンで頻出する表現を適切にグルーピングし、カード化・復習を最適化。
- **保守性強化**: ラベル追加に伴うマスターファイルと個別ファイルの同期プロセスを確立し、今後の拡張時の手戻りを削減。
